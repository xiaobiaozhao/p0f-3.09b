...
u8 auto_detect_link_off = 0;            /* auto detect link offset for vlan   */
...
"  -a        - auto detect link offset.(guess vlan) \n"
"\n"
...
static void prepare_pcap(void) {
  ...
  if (auto_detect_link_off){
    link_type = -1;
    
  }else{
    link_type = pcap_datalink(pt);
  }
  SAYF("[+] auto detect link offset %d, link_type %d. \n", auto_detect_link_off, link_type);
...
int main(int argc, char** argv) {
  ...
  while ((r = getopt(argc, argv, "+LS:daf:i:m:o:pr:s:t:u:")) != -1) switch (r) {
